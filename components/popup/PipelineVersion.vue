<template>
  <va-modal v-model="props.showPopup" title="파이프라인" mobile-fullscreen hide-default-actions size="large">
    <va-data-table :items="pipelineVersions ? pipelineVersions.result.pipeline_versions : []" :no-data-html="noItemText"
      :no-data-filtered-html="noItemText" :columns="pipelineVersionColumns" :per-page="pageSize"
...
watch(() => props.pipelineId, async (newId, oldId) => {
  // props.pipelineId = newId
  const versions = await getPipelineVersions(newId);
